...
//agora vamos fazer uma outra função que confira se você escreveu apenas o que é permitido (letras normais e espaço)
void ver_nome (char* nome){
  fgets(nome, 51, stdin);
  while (!valida_nome(nome)) {//se o valida_nome retorna 1, o loop acaba, se retornar 0, o loop continua pq ta com as logicas invertida
    printf("Caracteres inválidos\n");
    printf("Escreva o nome novamente\n\n");
...
void ver_cpf(char* cpf) {
    ...
    do {
        ...
        if (!valida_cpf(cpf)) {
            printf("CPF inválido. Escreva o CPF novamente.\n\n");
...
void ver_celular(char* celular) {
  ...
  while (!valida_celular(celular)) {
    printf("Caracteres inválidos\n");
    printf("Informe o celular novamente\n\n");
